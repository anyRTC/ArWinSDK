...
{
	int nBuildNumber = 0;
	const char *lpszEngineVer = getARRtcEngineVersion(&nBuildNumber);
...
BOOL CAgoraObject::JoinChannel(LPCTSTR lpChannelName, UINT lpUID, LPCSTR lpChannelToken)
{
	int nRet = 0;
...
//	m_lpAgoraEngine->setVideoProfile(VIDEO_PROFILE_720P);
	...
	nRet = m_lpAgoraEngine->joinChannel(lpChannelToken, szChannelName, NULL, szUserId);
#else
	nRet = m_lpAgoraEngine->joinChannel(lpChannelToken, lpChannelName, NULL, szUserId);
#endif
